...
def find_centroid(cluster):
    ...
    coordinates = [mean.lat, mean.long]
    return coordinates
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        new_centroids = group_by_centroid(restaurants, centroids)
        centroids = [find_centroid(r) for r in new_centroids]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    S_xy = sum([((a - mean(xs)) * (b - mean(ys))) for a, b in zip(xs, ys)])
    b = S_xy / S_xx
    a = mean(ys) - b * mean(xs)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for f in feature_fns:
        curr_r = find_predictor(user, reviewed, f)
        if curr_r[1] > max_r:
            max_r = curr_r[1]
            max_predict = curr_r[0]
    return max_predict
...
def rate_all(user, restaurants, feature_fns):
    ...
    predicted_rest_names  = [restaurant_name(r) for r in restaurants if r not in reviewed]
    ...
    #f: best_predictor(user, ALL_RESTAURANTS, f) for f in feature_fns}
